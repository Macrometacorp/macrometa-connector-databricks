...
class MacrometaMicroBatchStream(
  ...
  override def deserializeOffset(json: String): Offset =
    MacrometaOffset.fromJson(json)

  override def commit(end: Offset): Unit = {
    val endOffset = end.asInstanceOf[MacrometaOffset]
    if (
      lastAckedMessageId.isEmpty || !lastAckedMessageId.get.equals(
        endOffset.messageId
      )
    ) {
      ...
      case Right(components) =>
        MacrometaOffset(
          new MessageIdImpl(
            components.ledgerId,
            components.entryId,
            components.partitionIndex
          )
        )
      case Left(error) =>
        throw new IllegalArgumentException(
          s"Failed to deserialize MacrometaOffset from JSON: $error"
        )
    }
  }
}

case class MacrometaOffsetComponents(
    ledgerId: Long,
    entryId: Long,
    partitionIndex: Int
)
