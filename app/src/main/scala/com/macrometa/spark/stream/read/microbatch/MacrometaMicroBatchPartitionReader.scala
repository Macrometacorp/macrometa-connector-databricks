...
import org.apache.pulsar.client.api.{Consumer, Message, PulsarClient, SubscriptionType, Schema => PulsarSchema}
...
class MacrometaMicroBatchPartitionReader(options: Map[String, String], schema: StructType, partition: MacrometaInputPartition) extends PartitionReader[InternalRow] {

  private val rowEncoder = RowEncoder(schema).resolveAndBind()
  private val objectMapper = new ObjectMapper()
  private var currentMessage: Option[Message[Array[Byte]]] = None

  val client: PulsarClient = MacrometaPulsarClientInstance.getInstance(federation = options.getOrElse("regionurl",""),
    port = options.getOrElse("port",6651.toString), jwtToken = options.getOrElse("token","")).getClient
  ...
  override def next(): Boolean = {
    ...
    if (message != null && message.getMessageId.compareTo(partition.end.messageId) <= 0) {
